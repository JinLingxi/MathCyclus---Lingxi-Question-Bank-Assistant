"""Smoke test the shared exam-selection service on the legacy CSV index."""
...
def read_legacy_rows() -> list[dict[str, str]]:
    ...
    return [row for row in rows if str(row.get("试卷类型") or "").strip() != "WK"]
...
def main() -> int:
    ...
    checks = [
        check("legacy_csv_rows_returned", bool(rows), len(rows)),
        check(
            "legacy_csv_rows_have_selection_fields",
        ...
        check("writes_legacy_tex", True, False),
        check("writes_formal_database", True, False),
    ]
    ok = all(item["ok"] for item in checks)
    print(
        json.dumps(
            {
                "source_index": str(CSV_INDEX_PATH.relative_to(PROJECT_ROOT)),
